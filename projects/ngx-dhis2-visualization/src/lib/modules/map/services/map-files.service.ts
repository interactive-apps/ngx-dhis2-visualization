...
export class MapFilesService {
  ...
  downloadMapVisualizationAsCSV(mapVisualization) {
    const payload =
      mapVisualization.payload && mapVisualization.payload !== ''
        ? mapVisualization.payload
        : {};
    ...
    layers.forEach(layer => {
      ...
      const data = this._prepareCSVDataForDownload(
        analytics[layer.id],
        layer,
        legend,
  ...
  downloadMapVisualizationAsKML(mapVisualization) {
    ...
    layers.forEach((layer, layerIndex) => {
      const legend = _.find(legends, ['layer', layer.id]).legend;
      const geoJsonObject = this._prepareGeoJsonDataForDownload(
        geofeatures[layer.id],
        analytics[layer.id],
        legend
      );
  ...
  downloadMapVisualizationAsGeoJSON(mapVisualization) {
    ...
    layers.forEach((layer, layerIndex) => {
      ...
      const modifiedMapData = this._convertToBinaryData(
    ...
    if (data.hasOwnProperty('headers')) {
      ...
      if (settings.tyope === 'event') {
        ...
        data.rows.forEach(item => {
          const orgUnitCoordinateObject = this._getOrgUnitCoordinateObject(
            data.headers,
            item,
            geoFeatures
          );
          if (orgUnitCoordinateObject) {
            ...
            result += columnDelimiter;
            const classBelonged: any = this._getClass(
              data.headers,
              item,
              legend
            );
            result += classBelonged
              ? classBelonged.startValue + ' - ' + classBelonged.endValue
              : '';
            result += columnDelimiter;
            ...
            result += columnDelimiter;
            result += this._refineCoordinate(
              orgUnitCoordinateObject,
              orgUnitCoordinateObject.co
            );
            result += lineDelimiter;
          }
        });
  ...
  private _prepareGeoJsonDataForDownload(geoFeatures, analytics, legend) {
    ...
    geoJsonData = this._prepareGeoJsonDataFromGeoFeatures(
      geoFeatures,
      analytics,
      legend
    );
  ...
  private _prepareGeoJsonDataFromGeoFeatures(
    geoFeatures: any,
    analytics: any[],
    legend
  ): any {
  ...
  private _getGeoJSONObject(
    ...
    if (geoFeatures) {
      geoFeatures.forEach(geoFeature => {
         ...
         */
        if (analyticObject) {
          const dataElement = this._getDataForGeoFeature(
            geoFeature.id,
            analyticObject
          );
          if (dataElement) {
  ...
  private _getDataForGeoFeature(
    ...
    return !data.hasOwnProperty('id')
      ? this._getDefaultData(analyticObject)
      : data;
  }
    ...
    switch (organisantionUnit.ty) {
      ...
      case 2: {
        if (this.isMultiPolygon(organisantionUnit, rawCoordinates)) {
          wktCoordinate =
            '"MULTIPOLYGON((' +
            this.getMultiPolygonWKT(rawCoordinates) +
            '))' +
            '"';
        } else {
  ...
  private _root(_, options) {
    ...
    switch (_.type) {
      case 'FeatureCollection':
        ...
        return _.features
          .map(this._feature(options, styleHashesArray))
          .join('');
      case 'Feature':
  ...
  private _feature(options, styleHashesArray) {
    return _ => {
      ...
      if (options.simplestyle) {
        ...
        if (styleHash) {
          if (
            this.geometry.isPoint(_.geometry) &&
            this._hasMarkerStyle(_.properties)
          ) {
            ...
            (this.geometry.isPolygon(_.geometry) ||
              this.geometry.isLine(_.geometry)) &&
            this._hasPolygonAndLineStyle(_.properties)
          ) {
            if (styleHashesArray.indexOf(styleHash) === -1) {
              styleDefinition = this._polygonAndLineStyle(
                _.properties,
                styleHash
              );
              styleHashesArray.push(styleHash);
  ...
  private _documentName(options) {
    return options.documentName !== undefined
      ? this._tag('name', options.documentName)
      : '';
  }

  private _documentDescription(options) {
    return options.documentDescription !== undefined
      ? this._tag('description', options.documentDescription)
      : '';
  }

  private _name(_, options) {
    return _[options.name]
      ? this._tag('name', this._encode(_[options.name]))
      : '';
  }

  private _description(_, options) {
    return _[options.description]
      ? this._tag('description', this._encode(_[options.description]))
      : '';
  }
  ...
  private _timestamp(_, options) {
    return _[options.timestamp]
      ? this._tag(
          'TimeStamp',
          this._tag('when', this._encode(_[options.timestamp]))
        )
      : '';
  }
  ...
  private _getGeometry(): Object {
    const geometry = {
      Point: _ => {
        return this._tag(
          'Point',
          this._tag('coordinates', _.coordinates.join(','))
        );
      },
      LineString: _ => {
        return this._tag(
          'LineString',
          this._tag('coordinates', this._linearring(_.coordinates))
        );
      },
      Polygon: _ => {
        ...
        const outer = _.coordinates[0],
          ...
          outerRing = this._tag(
            ...
            this._tag(
              'LinearRing',
              this._tag('coordinates', this._linearring(outer))
            )
          ),
          innerRings = inner
            .map(i => {
              return this._tag(
                ...
                this._tag(
                  'LinearRing',
                  this._tag('coordinates', this._linearring(i))
                )
              );
            })
            .join('');
  ...
  private _data(_) {
    return this._tag('Data', this._tag('value', this._encode(_[1])), [
      ['name', this._encode(_[0])]
    ]);
  }
  ...
  private _markerStyle(_, styleHash) {
    return this._tag(
      ...
      this._tag(
        'IconStyle',
        this._tag('Icon', this._tag('href', this._iconUrl(_)))
      ) + this._iconSize(_),
      [['id', styleHash]]
    );
  }
  ...
  private _polygonAndLineStyle(_, styleHash) {
    const lineStyle = this._tag('LineStyle', [
      this._tag(
        'color',
        this._hexToKmlColor(_['stroke'], _['stroke-opacity']) || 'ff555555'
      ) +
        this._tag(
          'width',
          _['stroke-width'] === undefined ? 2 : _['stroke-width']
        )
    ]);
    ...
    if (_['fill'] || _['fill-opacity']) {
      polyStyle = this._tag('PolyStyle', [
        this._tag(
          'color',
          this._hexToKmlColor(_['fill'], _['fill-opacity']) || '88555555'
        )
      ]);
    }
  ...
  private _gmlFeature(options) {
    return _ => {
      ...
      return this._tag(
        'gml:featureMember',
        this._gmlFeatureAttributes(_, options)
      );
    };
  }
  ...
  private _preparePolygonMembers(coordinates): string {
    ...
    featureBlocks.forEach(featureBlock => {
      blockTag += this._tag(
        'gml:polygonMember',
        this._getPolygonOuterBoundTypeGmlFormat(featureBlock)
      );
    });
  ...
  private _getPointOuterBoundTypeGmlFormat(featureBlock): string {
    ...
    if (this._isFinalBlock(featureBlock)) {
      format = this._tag(
        ...
        this._tag(
          'gml:coordinates',
          this._convertfeatureCoordinateToGmlFormat(featureBlock)
        ),
        [['srsName', 'EPSG:4326']]
      );
    }
